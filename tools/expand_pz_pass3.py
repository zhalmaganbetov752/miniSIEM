# -*- coding: utf-8 -*-
"""Третий проход расширения (только EXTRA3)."""
...
def main() -> None:
    ...
    for key, next_prefix in ANCHORS:
        paragraphs = EXPANSIONS_EXTRA3.get(key)
        if not paragraphs:
            continue
    ...
    print(f"Pass3 inserted {inserted} paragraphs")
    print(f"Total chars: {chars} (~{chars // 2980} pages @2980)")
